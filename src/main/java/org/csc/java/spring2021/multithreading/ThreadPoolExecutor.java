...
public class ThreadPoolExecutor extends ThreadPoolBase implements ThreadPool{
    ...
    ThreadPoolExecutor(List<Thread> threads, ThreadsAlive threadsAlive, Running running, LinkedBlockingQueue<Runnable> tasks){
    ...
    public <T> ComposableFuture<T> invoke(Supplier<? extends T> action){
        ...
        tasks.add(task);
        return new ImpComposableFuture<>(result, this);
    }
    ...
    public void awaitFullShutdown() throws InterruptedException{
        ...
        while(threadsAlive.cntAlive.get() != 0){
            Thread.onSpinWait();
        }
    }
